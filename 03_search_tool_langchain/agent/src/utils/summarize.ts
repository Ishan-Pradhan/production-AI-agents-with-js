...
export async function summarize(text: string) {
    ...
    //ask model to summaruze
    const res = await model.invoke([
        new SystemMessage([
            "You are a helpful assistant that writes short, accurate summaries ",
            "Guidelines:",
            "- Be factual and neutral, avoid marketing languate.",
            "- 6-8 sentences; no lists unless absolutely necessary.",
            "- Do Not invent sources; you only summarize the provided text.",
            "- Keep it readable for beginners"

        ].join("\n")),

        new HumanMessage(["Summarize the following content for a beginner friendly audience.",
    ...
    const rawModelOutput = typeof res.content === "string"
        ? res.content
        : String(res.content);
